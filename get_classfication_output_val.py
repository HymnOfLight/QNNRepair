...
DEFAULT_INPUT_TFLITE = "./flatc_output/mobilenet_v2_1.0_224_quant.tflite"
parser = argparse.ArgumentParser()
parser.add_argument("-t", "--tflite", \
default= DEFAULT_INPUT_TFLITE, \
help="tflite model to be tested")
args = parser.parse_args()

interpreter = tf.lite.Interpreter(args.tflite)
tf.lite.experimental.Analyzer.analyze(args.tflite)
...
print(val_label)
with open(args.tflite + '.txt', 'w') as f1:
...
#                 print(output_data)
                ...
                if classifi_result - 1 == val_label[filecount]:
                    correct = correct + 1
                    print("correct")
                else:
                    print("incorrect")                                   
