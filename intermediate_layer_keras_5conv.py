...
model_path = "./original_model_conv5.h5"
...
with open('conv5_cifar10_float_result.txt', 'w') as f1:
            ...
            if os.path.isdir(d):
                for f in sorted(os.listdir(d)):
                    ...
                    if os.path.isfile(f):
                            ...
                            x = x.reshape(x.shape + (1,)) # add one extra dimension to the front
...
#                                     acts = get_activations(model, x, layer_names=layer.name)[layer.name]
                            #print(np.sign(acts))
                            save_path = '/local-data/e91868xs/quantized_model_conv5/'+ ora_class + "/" + os.path.split(f)[1] + '/'
...
#                                 output_filename = f'image_{image_file}_layer_{i}_output.txt'
                                np.savetxt(save_path+ os.path.split(f)[1] + '_resnet18_' +str(i)+'_tensor.txt', output.flatten())
