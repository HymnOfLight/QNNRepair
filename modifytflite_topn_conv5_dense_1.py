...
dstar_files = ["./conv5_dense_1_dstar.txt", "./conv5_dense_1_tarantula.txt", "./conv5_dense_1_ochiai.txt", "./conv5_dense_1_euclid.txt", "./conv5_dense_1_ample.txt", "./conv5_dense_1_wong3.txt", "./conv5_dense_1_jaccard.txt"]
n = 10
...
for topn in topn_values:
    for dstar_file in dstar_files:
        ...
        with open('./quantized_model_conv5.json', 'r') as tf_file:
            tflite_model_dict = json.load(tf_file)
            print(type(tflite_model_dict))
            print(tflite_model_dict.keys())
            for correct_target in maxn: 
                if os.path.exists('/local-data/e91868xs/resnet18_correction/conv5_dense1_#' + str(correct_target) + 'neuron_repair_result_with_all_pictures.txt'):
                    with open('/local-data/e91868xs/resnet18_correction/conv5_dense1_#' + str(correct_target) + 'neuron_repair_result_with_all_pictures.txt', 'r') as solution:
            ...
            with open('/local-data/e91868xs/conv5/json/conv5_dense_1_corrected_top'+ str(topn) + 'all_' + dstar_substring + '_with_all_images_maxn.json','w') as r:
    ...
    with open('./quantized_model_conv5.json', 'r') as tf_file:
        ...
        print(type(tflite_model_dict))
        print(tflite_model_dict.keys())
        for random_repeat in range(0,10):
            random_numbers = []
            for i in range(512):
            ...
            if start_index != -1 and end_index != -1:
                dstar_substring = dstar_file[start_index:end_index]
            with open('/local-data/e91868xs/conv5/json/vggnet_dense_1_corrected_top'+ str(topn) + 'random_#' + str(random_repeat) + 'all_with_all_images_maxn.json','w') as r:
                json.dump(tflite_model_dict,r)
            r.close()
