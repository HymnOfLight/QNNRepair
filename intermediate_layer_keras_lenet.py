...
model_path = "./original_lenet_model.h5"
...
with open('lenet_cifar10_float_result.txt', 'w') as f1:
...
#                                     acts = get_activations(model, x, layer_names=layer.name)[layer.name]
                            #print(np.sign(acts))
                            save_path = '/local-data/e91868xs/lenet_output/'+ ora_class + "/" + os.path.split(f)[1] + '/'
...
#                                 output_filename = f'image_{image_file}_layer_{i}_output.txt'
                                np.savetxt(save_path+ os.path.split(f)[1] + '_lenet_' +str(i)+'_tensor.txt', output.flatten())
