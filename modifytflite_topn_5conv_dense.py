...
dstar_files = ["./conv5_dense_dstar.txt", "./conv5_dense_tarantula.txt", "./conv5_dense_ochiai.txt", "./conv5_dense_euclid.txt", "./conv5_dense_ample.txt", "./conv5_dense_wong3.txt", "./conv5_dense_jaccard.txt"]
n = 512
...
for topn in topn_values:
    for dstar_file in dstar_files:
        ...
        with open('./quantized_model_conv5.json', 'r') as tf_file:
            ...
            for correct_target in maxn: 
                if os.path.exists('/local-data/e91868xs/conv5/correction/dense_conv3_#' + str(correct_target) + 'neuron_repair_result_with_all_pictures.txt'):
                    with open('/local-data/e91868xs/conv5/correction/dense_conv3_#' + str(correct_target) + 'neuron_repair_result_with_all_pictures.txt', 'r') as solution:
            ...
            with open('/local-data/e91868xs/conv5/json/conv5_dense_corrected_top'+ str(topn) + 'all_' + dstar_substring + '_with_all_images_maxn.json','w') as r:
    ...
    with open('./quantized_model_conv5.json', 'r') as tf_file:
        ...
        for random_repeat in range(0,20):
            ...
            if start_index != -1 and end_index != -1:
                dstar_substring = dstar_file[start_index:end_index]
            with open('/local-data/e91868xs/conv5/json/conv5_dense_corrected_top'+ str(topn) + 'random_#' + str(random_repeat) + 'all_with_all_images_maxn.json','w') as r:
                json.dump(tflite_model_dict,r)
            r.close()
