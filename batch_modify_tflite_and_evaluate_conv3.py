...
def find_tflite_files(folder_path):
    ...
    for root, dirs, files in os.walk(folder_path):
        for file in files:
            if "conv3_dense_corrected" in file and file.endswith(".tflite"):
                tflite_files.append(file)
    return tflite_files
directory = '/local-data/e91868xs/conv3_cifar/json'  
command_template = 'flatc -b --strict-json --defaults-json -o flatc_output tflite.fbs /local-data/e91868xs/conv3_cifar/json/'
...
with open('./conv3_dense_all_targets_neuron.txt', 'w') as tf_file:
