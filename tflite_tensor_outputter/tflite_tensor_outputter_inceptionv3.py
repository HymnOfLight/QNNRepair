...
def buffer_change_output_tensor_to(model_buffer, new_tensor_i):
  ...
  #root = tflite.Model.Model.GetRootAs(model_buffer, 0)
  ...
  # print(output_tensor_index_offset)
  #output_tensor_index_offset = 0x361A20 #mobilenetv2.tflite B33ED quant
  #output_tensor_index_offset = 0
...
if __name__ == "__main__":
  ...
  try:
    while True:
      ...
      ind += 1
      print(dets['name'])
      if "inception_v3/predictions/MatMul;inception_v3/predictions/BiasAdd" in dets['name']:
            if "quant" in args.model_file:
              # Write out to files: details and tensor            
              print("Writing {}/{}_weight_quant_details_{}.txt".format(subdir_name, ind, dets['name'].replace('/', '-')))
              print("Writing {}/{}_weight_quant_tensor.txt".format(subdir_name, ind))
              f_dets = open("{}/{}_weight_details_{}.txt".format(subdir_name, ind, dets['name'].replace('/', '-')), "w")
              f_dets.write("tensor[{}]: {}\n".format(ind, dets))
              # np.save("layer_outputs_dog/{}_tensor.txt".format(ind), tens)
              tens.tofile("{}/{}_weight_quant_tensor.txt".format(subdir_name, ind), ",")
              # pickle.dump(tens, f)
              # f.write("\n")
              ...
              # f_tens.close()
               ...
               # Write out to files: details and tensor
              print("Writing {}/{}_weight_details_{}.txt".format(subdir_name, ind, dets['name'].replace('/', '-')))
              print("Writing {}/{}_weight_tensor.txt".format(subdir_name, ind))
              f_dets = open("{}/{}_weight_details_{}.txt".format(subdir_name, ind, dets['name'].replace('/', '-')), "w")
              f_dets.write("tensor[{}]: {}\n".format(ind, dets))
              # np.save("layer_outputs_dog/{}_tensor.txt".format(ind), tens)
              tens.tofile("{}/{}_weight_loat_tensor.txt".format(subdir_name, ind), ",")
              # pickle.dump(tens, f)
              # f.write("\n")
              ...
              f_dets.close()
              # f_tens.close()
      if "inception_v3/avg_pool/Mean" in dets['name']:
            if "quant" in args.model_file:
              # Write out to files: details and tensor            
              ...
              f_dets.write("tensor[{}]: {}\n".format(ind, dets))
              # np.save("layer_outputs_dog/{}_tensor.txt".format(ind), tens)
              tens.tofile("{}/{}_float_tensor.txt".format(subdir_name, ind), ",")
              # pickle.dump(tens, f)
              # f.write("\n")
