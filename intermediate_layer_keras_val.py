...
# model3 = tf.keras.applications.mobilenet_v2.MobileNetV2(
#     input_shape=None,
#     alpha=1.0,
#     include_top=True,
#     weights='imagenet',
...
overall = 0
model.summary()
model.save("./model/DenseNet121fullprecision.h5")
...
with open('./model/DenseNet121_weight_quantized.tflite', 'wb') as f:
...
# tflite_quant_model = converter.convert()
with open('DenseNet121_float_result.txt', 'w') as f1:
            ...
            if os.path.isdir(d):
                for f in sorted(os.listdir(d)):
                    ...
                    if os.path.isfile(f):
                            ...
                            relu_layer = 0
                            for layer in model.layers:                          
                                if 'avg_pool' in layer.name:
                                    relu_layer+=1                               
                                    #print(get_activations(model, x, layer_names=layer.name))
                                    acts = get_activations(model, x, layer_names=layer.name)[layer.name]
                                    #print(np.sign(acts))
                                    save_path = './output_densenet/' + os.path.split(f)[1] + '/'
                                    if not os.path.exists(save_path):
                                        os.makedirs(save_path)
                                    np.savetxt(save_path+ os.path.split(f)[1] + "_DenseNet121_" +str(relu_layer)+"_"+layer.name+'_tensor.txt',np.array(acts).reshape(-1),delimiter=',')
                                    np.savetxt(save_path+os.path.split(f)[1] + "_DenseNet121_" +str(relu_layer)+"_"+layer.name+'_activation_status_tensor.txt',np.sign(np.array(acts)).reshape(-1),delimiter=',')
