...
for topn in topn_values:
    for dstar_file in dstar_files:
        ...
        with open('./quantized_vggnet_model.json', 'r') as tf_file:
            ...
            with open('/local-data/e91868xs/vggnet/json/vggnet_dense_1_corrected_top'+ str(topn) + 'all_' + dstar_substring + '_with_all_images_maxn.json','w') as r:
    ...
    with open('./quantized_vggnet_model.json', 'r') as tf_file:
        ...
        for random_repeat in range(0,10):
            ...
            if start_index != -1 and end_index != -1:
                dstar_substring = dstar_file[start_index:end_index]
            with open('/local-data/e91868xs/vggnet/json/vggnet_dense_1_corrected_top'+ str(topn) + 'random_#' + str(random_repeat) + 'all_with_all_images_maxn.json','w') as r:
                json.dump(tflite_model_dict,r)
            r.close()
