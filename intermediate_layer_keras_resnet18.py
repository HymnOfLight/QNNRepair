...
model_path = "./original_resnet18_model.h5"
...
with open('resnet18_cifar10_float_result.txt', 'w') as f1:
...
#                                     acts = get_activations(model, x, layer_names=layer.name)[layer.name]
                            #print(np.sign(acts))
                            save_path = '/local-data/e91868xs/resnet50_cifar/'+ ora_class + "/" + os.path.split(f)[1] + '/'
