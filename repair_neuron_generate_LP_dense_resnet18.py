...
dir = -1
pathdir = "/local-data/e91868xs/resnet50_cifar/"
weight_file = "./models/resnet18/ResNet18denseMatMul.npy"
dense_weight = np.load(weight_file)
print(dense_weight.shape)
bias_file = "./models/resnet18/ResNet18denseBiasAddReadVariableOp.npy"
    ...
    while stack:
        ...
        for filename in os.listdir(current_dir):
            ...
            if os.path.isfile(filepath) and filepath.endswith(".txt"):               
                ...
                if "76_weight_quant_tensor.txt" in filepath:
                    ...
                    if classifi_result != index:
                        ...
                        last_input_q = os.path.join(os.path.dirname(filepath), "75_weight_quant_tensor.txt")
                        last_input_f = os.path.join(os.path.dirname(filepath), folder_name + "_resnet18_67_tensor.txt")
                        last_output_q = os.path.join(os.path.dirname(filepath), "76_weight_quant_tensor.txt")
                        last_output_f = os.path.join(os.path.dirname(filepath), folder_name + "_resnet18_68_tensor.txt")
                        f_results = os.path.join(os.path.dirname(filepath), folder_name + "_resnet18_68_tensor.txt")                     
...
#     m.write('repair_dense_conv3_#' + str(neuron_index) + 'neuron.lp')
    ...
    if m.Status == GRB.OPTIMAL:
        ...
        with open('/local-data/e91868xs/resnet18_correction/dense_resnet18_#' + str(neuron_index) + 'neuron_repair_result_with_all_pictures.txt', 'w') as f1:
