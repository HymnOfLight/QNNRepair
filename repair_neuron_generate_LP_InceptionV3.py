...
weight_file = "./InceptionV3_weight_file.npy"
conv_weight = np.load(weight_file)
print(conv_weight.shape)
bias_file = "./InceptionV3_bias.npy"
bias = np.load(bias_file)
#neuron_index = 108

for neuron_index in range (0,1000):
    image_count = 0
    m = gp.Model('neuron')
    ...
    #         for f in os.listdir(d):
            for f in sorted(os.listdir(d)):           
                ...
                #print(f)        
                if os.path.isfile(f):
                    if "316_quant_tensor" in f:                   
                        if result_diff[dir] == 0:
                            image_count = image_count + 1
                            last_input = np.loadtxt(f, dtype=np.int8, delimiter=',')
#                             print(last_input[0])
#                             print(conv_weight.shape)
#                             print(bias.shape)
                            ...
                            #print((gp.quicksum((conv_weight[neuron_index][0][0][i]+vars[i]) * last_input[i] for i in range(0, 1279))+ bias[neuron_index])*(gp.quicksum((conv_weight[neuron_index][0][0][i]) * last_input[i] for i in range(0, 1279))+ bias[neuron_index])<=0)
                            m.addConstr((gp.quicksum((conv_weight[neuron_index][i]+vars[i]) * last_input[i] for i in range(0, 2047))+ bias[neuron_index])*(gp.quicksum((conv_weight[neuron_index][i]) * last_input[i] for i in range(0, 2047))+ bias[neuron_index])<=0)
        ...
        if image_count > 100: #max94
            break
    m.write('./correction/InceptionV3_repair_#' + str(neuron_index) + 'neuron.lp')
    m.optimize()

    if m.Status == GRB.OPTIMAL:
        solution = m.getAttr('X', vars)
        print(solution)
        with open('./correction/InceptionV3_#' + str(neuron_index) + 'neuron_repair_result_with_all_pictures.txt', 'w') as f1:
