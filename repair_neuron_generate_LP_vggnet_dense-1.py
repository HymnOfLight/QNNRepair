...
weight_file = "./models/vggnet/modelfc6-bnbatchnormmul_1.npy"
dense_weight = np.load(weight_file)
print(dense_weight.shape)
bias_file = "./models/vggnet/modelfc6-reluRelu;modelfc6-bnbatchnormadd_1.npy"
bias = np.load(bias_file)
#neuron_index = 108
print(dense_weight.shape)
for neuron_index in range (0,512):
    image_count = 0
    m = gp.Model('neuron')
                ...
                if "47_weight_quant_tensor" in filepath:
                    ...
                    if classifi_result != index:
                        ...
                        if f_classifi_result == index:
                            ...
                            m.addConstr((gp.quicksum((dense_weight[neuron_index][i]+vars[i]) * last_input[i] for i in range(0, 2047))+ bias[neuron_index])*last_output_dense_f[neuron_index]>=0)
...
#     m.write('repair_dense_conv3_#' + str(neuron_index) + 'neuron.lp')
    ...
    if m.Status == GRB.OPTIMAL:
        ...
        print(solution)
        with open('/local-data/e91868xs/vggnet/correction/dense-1_vggnet_#' + str(neuron_index) + 'neuron_repair_result_with_100_pictures.txt', 'w') as f1:
