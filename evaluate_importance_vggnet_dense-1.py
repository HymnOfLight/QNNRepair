...
activate_diff_all_cnas = np.zeros(512)
activate_diff_all_cnaf = np.zeros(512)
activate_status_quant = np.zeros(512)
activate_status_float = np.zeros(512)
dstar = np.zeros(512)
tarantula = np.zeros(512)
jaccard = np.zeros(512)
ochiai = np.zeros(512)
ample = np.zeros(512)
euclid = np.zeros(512)
wong3 = np.zeros(512)
h = np.zeros(512)
activate_diff = np.zeros(512)
filecount = -1
stack = [pathdir]
while stack:
        ...
        if os.path.isfile(filepath) and filepath.endswith(".txt"):               
            ...
            if "47_weight_quant_tensor.txt" in filepath:
                ...
                if classifi_result != index:
                    ...
                    last_input_q = os.path.join(os.path.dirname(filepath), "44_weight_quant_tensor.txt")
                    last_input_f = os.path.join(os.path.dirname(filepath), folder_name + "_vggnet_41_tensor.txt")
                    last_output_q = os.path.join(os.path.dirname(filepath), "45_weight_quant_tensor.txt")
                    last_output_f = os.path.join(os.path.dirname(filepath), folder_name + "_vggnet_42_tensor.txt")
                    f_results = np.loadtxt(os.path.join(os.path.dirname(filepath), folder_name + "_vggnet_44_tensor.txt"), dtype=np.float32, delimiter='\n')
                    q_results = np.loadtxt(os.path.join(os.path.dirname(filepath), "47_weight_quant_tensor.txt"),dtype=np.float32, delimiter=',') 
...
with open('vggnet_dense-1_importance_result.txt', 'w') as f1:
    print("dstar")
#     print(dstar)
#     print(np.argmax(dstar))
    ...
    f1.write("\n")
    np.savetxt('vggnet_dense-1_dstar.txt',dstar)
    print("jaccard")
#     print(jaccard)
#     print(np.argmax(jaccard))
    ...
    f1.write("\n")
    np.savetxt('vggnet_dense-1_jaccard.txt',jaccard)
    print("tarantula")
#     print(tarantula)
#     print(np.argmax(tarantula))
    ...
    f1.write("\n")
    np.savetxt('vggnet_dense-1_tarantula.txt',tarantula)
    print("ochiai")
#     print(ochiai)
#     print(np.argmax(ochiai))
    ...
    f1.write("\n")
    np.savetxt('vggnet_dense-1_ochiai.txt',ochiai)
    print("euclid")
#     print(euclid)
#     print(np.argmax(euclid))
    ...
    f1.write("\n")
    np.savetxt('vggnet_dense-1_euclid.txt',euclid)
    print("ample")
#     print(ample)
#     print(np.argmax(ample))
    ...
    f1.write("\n")
    np.savetxt('vggnet_dense-1_ample.txt',ample)
...
#     print(np.argmax(wong3))
    ...
    f1.write("\n")
    np.savetxt('vggnet_dense-1_wong3.txt',wong3)
